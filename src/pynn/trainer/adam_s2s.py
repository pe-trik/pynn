...
def train_epoch_with_sample_recording(model, data, opt, eps, device, b_update, b_sync, n_print,
                teacher_force=1., weight_noise=False, grad_clip=0., grad_norm=False, fp16=False):
    ''' Epoch operation in training phase'''
    model.train()
    ...
    for batch_i, batch in enumerate(loader):
        # prepare data
        src_seq, src_mask, tgt_seq, sample_src_seq, sample_src_mask, sample_tgt_seq  = map(lambda x: x.to(device), batch)
        gold = tgt_seq[:, 1:]
        tgt_seq = tgt_seq[:, :-1]
        last = (batch_i == data_len)
        n_seq += tgt_seq.size(0)
        ...
        try:
            ...
            with autocast(enabled=fp16):
                ...
                sampling = teacher_force < 1.
                if sampling:
                    pred, src_seq, src_mask = model(src_seq, src_mask, tgt_seq, sample_src_seq, sample_src_mask, sample_tgt_seq)            
                    pred = torch.argmax(pred, dim=-1).detach()
                    sample = pred.clone().bernoulli_(1. - teacher_force)
                    pred = pred * sample
                    pred = torch.cat((tgt_seq[:, :1], pred[:, :-1]), dim=1)
                ...
                pred = model(src_seq, src_mask, tgt_seq, sample_src_seq, sample_src_mask, sample_tgt_seq, encoding=not sampling)[0]
                pred = pred.view(-1, pred.size(2))
                loss, loss_data, n_correct, n_token = cal_ce_loss(pred, gold, eps)
                if torch.isnan(loss.data):
                    print("    inf loss at %d" % n_seq); continue
...
def eval_epoch_with_sample_recording(model, data, device, fp16=False):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(loader):
            ...
            # forward
            with autocast(enabled=fp16):
                pred = model(src_seq, src_mask, tgt_seq, sample_src_seq, sample_src_mask, sample_tgt_seq)[0]
...
def train_model(model, datasets, epochs, device, cfg, fp16=False, dist=False,with_sample_recording=False):
    ''' Start training '''
    ...
    epoch_train = train_epoch_with_sample_recording if with_sample_recording else train_epoch
    epoch_eval = eval_epoch_with_sample_recording if with_sample_recording else eval_epoch
    while epoch_i < epochs:
        if not test:
            ...
            start = time.time()
            tr_loss, tr_accu = epoch_train(model_opt, tr_data, opt, eps, device, b_update, b_sync, n_print,
        ...
        start = time.time()
        cv_loss, cv_accu = epoch_eval(model_opt, cv_dat, device, fp16=fp16)
