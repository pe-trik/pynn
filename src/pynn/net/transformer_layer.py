...
class PositionwiseFF(nn.Module):
    def __init__(self, d_in, d_hid, dropout=0.1, residual=False, norm=True):
        super().__init__()
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, d_inner, n_head, dropout=0.1, rel_pos=False, adapter_names=None, d_adapter=512):
        super().__init__()
        ...
        if adapter_names:
            self.adapters = torch.ModuleDict()
            self.adapter_norm = nn.LayerNorm(d_model)
            for adapter in adapter_names:
                self.adapters[adapter] = PositionwiseFF(d_model, d_adapter, residual=False, norm=False)

    def forward(self, enc_inp, slf_mask=None, scale=1., adapter=None):
        enc_out = self.slf_attn(enc_inp, mask=slf_mask, scale=scale)[0]
        enc_out = self.pos_ffn(enc_out, scale=scale)
        if self.adapter_names:
            residual = enc_out
            enc_out = self.adapters[adapter](enc_out)
            enc_out = residual + self.adapter_norm(enc_out)
        return enc_out

class DecoderLayer(nn.Module):
    def __init__(self, d_model, d_inner, n_head, dropout=0.1, n_enc_head=0, rel_pos=False, adapter_names=None, d_adapter=512):
        super().__init__()
        Attention = RelPositionAttention if rel_pos else MultiHeadedAttention
        self.slf_attn = Attention(n_head, d_model, dropout, residual=True)
    ...
    def forward(self, dec_inp, enc_out, slf_mask=None, dec_enc_mask=None, scale=1., adapter=None):
