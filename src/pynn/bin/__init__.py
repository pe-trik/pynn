...
from pynn.io.audio_seq import SpectroDataset, SpectroDatasetWithSampleRecording
...
def train_s2s_model_with_sample_recording(model, args, device, n_device=1):
    dist, verbose = n_device > 1, device == 0
    tr_data = SpectroDatasetWithSampleRecording(args.train_scp, args.train_target, downsample=args.downsample,
    ...
    cv_data = SpectroDatasetWithSampleRecording(args.valid_scp, args.valid_target, downsample=args.downsample,
                             sort_src=True, mean_sub=args.mean_sub, var_norm=args.var_norm,
                             fp16=args.fp16, preload=args.preload, threads=2, verbose=verbose)
    ...
    train_s2s(model, datasets, args.n_epoch, device, cfg, fp16=args.fp16, dist=dist, with_sample_recording=True)
